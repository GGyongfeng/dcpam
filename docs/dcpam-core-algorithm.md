...
第二步，用相机内参把像素点反投影到接收屏上的三维实像点。2601.05 文档中的
基础写法是使用固定接收屏深度 `Z_s`：

```text
P_f_real       = Z_s * K_1^-1 * p_1
P_b_real_in_C2 = Z_s * K_2^-1 * p_2
```

结合当前《标定数据》文档，后续实现可以把固定 `Z_s` 升级为“相机射线与
接收屏标定平面求交”。这样更能利用已标定的前、后接收屏外参。

第三步，把接收屏实像点转换到虚像面点。2601.05 文档中使用 `H_1`、`H_2`
表达绕旋转中心的刚体变换：

```text
P_f_virtual       = H_1 * P_f_real
P_b_virtual_in_C2 = H_2 * P_b_real_in_C2
```

结合当前《标定数据》文档，后续实现可以用“关于反射屏标定平面的镜像矩阵”
替代固定角度的 `H_1`、`H_2`。这可以避免把 45 度安装假设写死到算法里。
...
2601.05 文档把两路像素点到虚像点的全过程整理为复合矩阵：

```text
P_f_virtual = H_1 * [Z_s * K_1^-1 * p_1, 1]^T
P_b_virtual = T_C2_to_C1 * H_2 * [Z_s * K_2^-1 * p_2, 1]^T
```

展开后，两个虚像点坐标都可以看作像素圆心坐标的仿射函数：
...
相机外参矩阵 `E`、空间平面在相机坐标系中的位置、`Z_s`、`H_1`、`H_2`
以及后相机到前相机的相对位姿。

当前项目后续实现可以按两层模型理解：

```text
旧模型: pixel -> Z_s 反投影 -> H_1/H_2 刚体变换 -> T_C2_to_C1 -> distance
新模型: pixel -> 接收屏平面求交 -> 反射屏平面镜像 -> T_C2_to_C1 -> distance
```

两者的接口形状相同，都是从两个像素圆心输出统一坐标系下的两枚虚像点。
区别在于新模型让 `Z_s`、旋转中心和 45 度角假设退居为标定数据的结果，
几何计算直接由接收屏与反射屏平面驱动。
