...
新方案 reproj 好一个数量级、法线符合规格；σ 与旧方案的关系见下一节的公平对照
（此前用的"旧 σ=174"是不公平的基线）。

---

## 6.5 公平对照：旧方法用真实尺寸后 σ 也变差

新方案 σ=206μm 一度看似"比旧方法差 30μm"，但那个旧基线（≈174~189μm）是用
**错误的取景框尺寸 22×17** 跑出来的——而实测取景框是 **25×20**。尺寸偏小会让
矩形 PnP 解出的深度偏近（相似三角形），这个 bug 顺带影响了 σ。

用 `git worktree` 拉出旧矩形代码，只改尺寸做对照（同一套 `dataset/old-pnp` 标定图、
同一批 samples 像素）：

| 方法 | 成像面深度 z | 平均 σ |
|---|---|---|
| 旧矩形 **22×17**（含 bug，原基线） | 29.6mm | 189.3μm |
| 旧矩形 **25×20**（真实尺寸，公平） | 34.1mm | 203.7μm |
| **新 5 圆点** | 37.4mm | **206.2μm** |

两个结论：
1. **尺寸修正验证了深度假说**：22×17 → 25×20，深度从 29.6 → 34.1mm（更远），
   方向和幅度都与"新方案 37mm"一致（`31×20/17≈36.8`）。**旧标定深度偏近，
   就是 22×17 尺寸 bug 导致的。**
2. **公平对照下新旧持平**：旧方法改用真实尺寸后 σ 从 189 变差到 204μm——说明
   旧方法"看起来好"部分是靠尺寸 bug。**新 5 圆点（206）≈ 旧矩形真实尺寸（204），
   几乎持平**，而新方法 reproj 好一个数量级、姿态跨 32 图 std=0.001、法线朝向正确。

**所以"新方法变差"是假象**：拿的是沾了尺寸 bug 光的旧基线。公平比较下新方法在
相同 σ 水平上几何全面更优，且 σ 还可通过靶点重标定进一步降低（已验证靶点
x+6mm → 177μm，因为旧标定的错误倾斜曾"抵消"靶点位置误差，新标定几何正确、
不再抵消，于是暴露出靶点本身的误差）。
...
6. **对照基线必须公平**。旧基线本身带一个尺寸 bug（22×17），拿它当参照会得出
   "新方法变差"的错误结论。改用真实尺寸重跑旧方法（`git worktree` 隔离旧代码）
   才发现新旧持平。比较前先确认基线没有已知缺陷。
7. **σ 好可能是"错误互相抵消"**。旧标定的错误倾斜恰好补偿了靶点位置误差，
   使 σ 看着好。几何正确的标定反而暴露真实误差——此时该修误差源（靶点），
   而非回退到"碰巧抵消"的错误标定。
...
## 10. 下一步方向：几何自标定（Geometric Self-Calibration）

**核心思想**：能让重复度 σ 最小的那组几何参数，就是真实的几何尺寸。
因为几何参数与物理真值一致时，同一点的多次测量收敛到同一值（σ→0）；
参数偏离真值则引入随姿态变化的系统偏差，把 σ 撑大。所以 **σ 是几何参数正确性的
目标函数**（bundle adjustment / self-calibration 思想）。

现成基础设施：`scripts/30_mirror_sensitivity_mc.py` 已实现"扰动反射镜位置/角度
（10 维）→ 看 σ 如何变"的蒙特卡洛 + 单变量扫描框架，可作为自标定的起点扩展。

**方案（下次实现）**：
- 目标函数：`最小化 组内 σ 均值 + 真值约束正则`。正则项把已知实测真值
  （关节臂实测间距 79.74mm、反射镜 45.22°、杆长等，见 `docs/device-physical-spec.md`）
  作为软约束，**防止参数互相抵消 / 跑飞到非物理值**。
- 优化变量（分阶段放开，避免过拟合/参数不可辨识）：
  反射镜 pose（10 维）→ 靶点/杆长 → 成像面几何（间距、深度）。
- 方法：蒙特卡洛搜索粗定位 + 局部优化（如 scipy.minimize）精修，或 ML。

**关键陷阱（本次实验的直接教训）**：σ 最小 ≠ 唯一真值——放开太多自由度一起
优化 σ，优化器会找到"整体抵消"的参数组（σ 很小但每个参数都不是物理真值，
正如旧标定的错误倾斜抵消靶点误差）。必须靠真值约束正则 + 分阶段放开变量 +
交叉验证（留出部分组做验证集）来保证参数可辨识、物理可信。
